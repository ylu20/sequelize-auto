...
var camelize = function (str) {
  return str.replace(/(?:_[A-Za-z])/g, function(letter, index) {
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      text[table] += spaces + "return sequelize.define('" + camelize(table) + "', {\n";
      ...
      _.each(fields, function(field, i){
        ...
        text[table] += spaces + spaces + camelize(field) + ": {\n"+spaces+spaces+spaces+" field:\""+field+"\",\n";
...
AutoSequelize.prototype.write = function(attributes, callback) {
  ...
  function createFile(table, _callback){
    fs.writeFile(path.resolve(path.join(self.options.directory, camelize(table) + '.js')), attributes[table], _callback);
  }
}
